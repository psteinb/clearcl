...
public enum MemAllocMode
{
  ...
  /**
   * CL_MEM_USE_HOST_PTR: Buffer was already created in existing application
   * code and the alignment and size rules were followed when the buffer was
   * allocated, or you want control over the buffer allocation and do not want
   * to rely on OpenCL. In cases when you don't want to incur the cost of a copy
