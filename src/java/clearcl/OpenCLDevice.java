...
import clearcl.utils.JavaCLUtils;
...
public class OpenCLDevice implements AutoCloseable
{
	...
	public CLKernel compileKernel(final URL url, final String kernelName)
	{
		...
		try
		{
			...
			System.out.println("Include path: " + url.getPath()
																								.substring(	0,
																														url.getPath()
																																.lastIndexOf(File.separator)));
			mCLProgram.addInclude(url.getPath()
																.substring(	0,
																						url.getPath()
																								.lastIndexOf(File.separator)));
	...
	public CLEvent[] runSubdivisions(	final CLKernel pKernel,
																		final int mNx,
																		final int mNy,
																		int subdivisions)
	{
		...
		for (int i = 0; i < subdivisions; i++)
		{
			...
			{
				evt[i + subdivisions * j] = pKernel.enqueueNDRange(	mCLQueue,
																														new long[]
																														{ i		* sizeX,
																															j		* sizeY },
																														new long[]
