...
public class Mandelbrot
{
  ...
  private static final double WIDTH = 4.0, HEIGHT = 3.0;
  ...
  static
  {
    try
    {
      source = ioResourceToByteBuffer("kernel/Mandelbrot.cl", 4096);
    }
    catch (IOException e)
    {
  ...
  /**
   * The event callbacks run on the main thread. We use this queue to apply any
   * changes in the rendering thread.
   */
  ...
  private double offsetX = -0.5, offsetY;
  ...
  private boolean syncGLtoCL; // false if we can make GL wait on events
                              // generated from CL queues.
  private long clEvent;
  private long glFenceFromCLEvent;

  private boolean syncCLtoGL; // false if we can make CL wait on sync objects
                              // generated from GL.
  ...
  public Mandelbrot(CLPlatform platform,
                    GLFWWindow window,
                    int deviceType,
                    boolean debugGL,
                    int maxIterations)
  {
    ...
    nglfwGetWindowSize(window.handle,
                       memAddress(size),
                       memAddress(size) + 4);
    ww = size.get(0);
    wh = size.get(1);

    nglfwGetFramebufferSize(window.handle,
                            memAddress(size),
                            memAddress(size) + 4);
    fbw = size.get(0);
    fbh = size.get(1);
    ...
    try
    {
      ...
      List<CLDevice> devices = platform.getDevices(deviceType,
                                                   glSharingFilter);
      if (devices == null)
      {
      ...
      clContext = clCreateContext(ctxProps,
                                  device.address(),
                                  clContextCB = new CLContextCallback()
                                  {
                                    @Override
                                    public void invoke(long errinfo,
                                                       long private_info,
                                                       long cb,
                                                       long user_data)
                                    {
      ...
      initColorMap(colorMapBuffer,
                   32,
                   Color.BLUE,
                   Color.GREEN,
                   Color.RED);

      clColorMap = clCreateBuffer(clContext,
      ...
      clQueue = clCreateCommandQueue(clContext,
                                     device.address(),
                                     0L,
                                     errcode_ret);
      checkCLError(errcode_ret);
      ...
      log("Max Iterations: " + maxIterations
          + " (Use -iterations <count> to change)");
      log("Display resolution: " + ww
          + "x"
          + wh
          + " (Use -res <width> <height> to change)");
      ...
      log(syncGLtoCL ? "GL to CL sync: Using clFinish"
      ...
      FloatBuffer quad = BufferUtils.createFloatBuffer(4 * 4)
                                    .put(new float[]
                                    { 0.0f,
                                     0.0f,
                                     0.0f,
      ...
      glShaderSource(vsh,
                     "#version 150\n" + "\n"
                         ...
                         + "}");
      glCompileShader(vsh);
      String log = glGetShaderInfoLog(vsh,
                                      glGetShaderi(vsh,
                                                   GL_INFO_LOG_LENGTH));
      if (!log.isEmpty())
      ...
      glShaderSource(fsh,
                     "#version 150\n" + "\n"
                         ...
                         + "}");
      glCompileShader(fsh);
      log = glGetShaderInfoLog(fsh,
                               glGetShaderi(fsh, GL_INFO_LOG_LENGTH));
      if (!log.isEmpty())
      ...
      log = glGetProgramInfoLog(glProgram,
                                glGetProgrami(glProgram,
                                              GL_INFO_LOG_LENGTH));
      if (!log.isEmpty())
      ...
      projectionUniform = glGetUniformLocation(glProgram,
                                               "projection");
    ...
    glfwSetWindowSizeCallback(window.handle,
                              window.windowsizefun = new GLFWWindowSizeCallback()
                              {
                                @Override
    ...
    glfwSetKeyCallback(window.handle,
                       window.keyfun = new GLFWKeyCallback()
                       {
                         @Override
                         public void invoke(long window,
                                            int key,
                           ...
                           case GLFW_KEY_D:
                             events.offer(new Runnable()
                             {
                               ...
                               public void run()
                               {
                                 ...
                                 log("DOUBLE PRECISION IS NOW: " + (doublePrecision ? "ON"
                                                                                   : "OFF"));
                                 rebuild = true;
                               }
                             });
    ...
    glfwSetMouseButtonCallback(window.handle,
                               window.mousebuttonfun = new GLFWMouseButtonCallback()
                               {
                                 @Override
                                 public void invoke(long window,
                                                    int button,
    ...
    glfwSetCursorPosCallback(window.handle,
                             window.cursorposfun = new GLFWCursorPosCallback()
                             {
                               @Override
                               public void invoke(long window,
                                                  double xpos,
    ...
    glfwSetScrollCallback(window.handle,
                          window.scrollfun = new GLFWScrollCallback()
                          {
                            @Override
                            public void invoke(long window,
                                               double xoffset,
                                               double yoffset)
                            {
                              ...
                              zoom *= (1.0 - yoffset * (ctrlDown ? 0.25
                                                                : 0.05));
  ...
  void renderLoop()
  {
    ...
    while (glfwWindowShouldClose(window.handle) == GLFW_FALSE)
    {
      ...
      else
      {
        ...
        log(String.format("%s: %d frames in 5 seconds = %.2f",
                          clGetPlatformInfoStringUTF8(platform.address(),
                                                      CL_PLATFORM_VENDOR),
                          fps,
                          fps / (timeUsed / 1000f)));
        fps = 0;
  ...
  private void computeCL(boolean is64bit)
  {
    ...
    int errcode = clEnqueueAcquireGLObjects(clQueue,
                                            clTexture,
                                            null,
                                            null);
    checkCLError(errcode);

    errcode = clEnqueueNDRangeKernel(clQueue,
                                     clKernel,
                                     2,
                                     null,
    ...
    errcode = clEnqueueReleaseGLObjects(clQueue,
                                        clTexture,
                                        null,
                                        !syncGLtoCL ? syncBuffer
                                                   : null);
    checkCLError(errcode);
    ...
    if (!syncGLtoCL)
    {
      ...
      glFenceFromCLEvent = glCreateSyncFromCLeventARB(clContext,
                                                      clEvent,
                                                      0);
    }
  ...
  private void buildProgram()
  {
    ...
    int errcode = clBuildProgram(clProgram,
                                 cl_device_id,
                                 options,
                                 buildCallback = new CLProgramCallback()
                                 {
                                   @Override
                                   public void invoke(long program,
                                                      long user_data)
                                   {
                                     System.err.printf("The cl_program [0x%X] was built %s\n",
                                                       program,
                                                       clGetProgramBuildInfoInt(program,
                                                                                cl_device_id,
                                                                                CL_PROGRAM_BUILD_STATUS) == CL_SUCCESS ? "successfully"
                                                                                                                      : "unsuccessfully");
                                     String log = clGetProgramBuildInfoStringASCII(program,
                                                                                   cl_device_id,
  ...
  private void initGLObjects()
  {
    ...
    clTexture = clCreateFromGLTexture2D(clContext,
                                        CL_MEM_WRITE_ONLY,
                                        GL_TEXTURE_2D,
                                        0,
                                        glTexture,
  ...
  private static void glOrtho(float l,
                              float r,
                              float b,
                              float t,
                              float n,
  ...
  private static void initColorMap(IntBuffer colorMap,
                                   int stepSize,
                                   Color... colors)
  {
