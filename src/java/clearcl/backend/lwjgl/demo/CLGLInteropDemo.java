  ...
  public static void main(String... args)
  {
    ...
    Collections.sort(platforms, new Comparator<CLPlatform>()
    {
      ...
      public int compare(CLPlatform p1, CLPlatform p2)
      {
        ...
        return clGetPlatformInfoStringUTF8(p1.address(),
                                           CL_PLATFORM_VENDOR).compareTo(clGetPlatformInfoStringUTF8(p1.address(),
                                                                                                     CL_PLATFORM_VENDOR));
      }
    });
    ...
    if (platform.getCapabilities().cl_khr_icd)
      platformID = clGetPlatformInfoStringASCII(platform.address(),
                                                CL_PLATFORM_ICD_SUFFIX_KHR); // less
                                                                             // spammy
    else
      platformID = clGetPlatformInfoStringUTF8(platform.address(),
                                               CL_PLATFORM_VENDOR);
    ...
    {
      long type = clGetDeviceInfoLong(device.address(),
                                      CL_DEVICE_TYPE);
    ...
    {
      final int deviceType = i == 1 || !hasGPU ? CL_DEVICE_TYPE_CPU
                                              : CL_DEVICE_TYPE_GPU;

      String ID = platformID + " - "
                  + (deviceType == CL_DEVICE_TYPE_CPU ? "CPU" : "GPU");
      final GLFWWindow window = new GLFWWindow(glfwCreateWindow(initWidth,
                                                                initHeight,
                                                                ID,
                                                                NULL,
                                                                NULL),
