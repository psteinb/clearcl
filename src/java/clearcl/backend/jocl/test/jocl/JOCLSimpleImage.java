...
import static org.jocl.CL.clCreateImage2D;
...
public class JOCLSimpleImage
{
  ...
  /**
   * The source code of the kernel to execute. It will rotate the input image by
   * the given angle and write the result into the output image.
   */
   ...
   */
  private static BufferedImage createBufferedImage(Class<?> pClass,
                                                   String fileName)
  {
    BufferedImage image = null;
    try
    {
      image = ImageIO.read(pClass.getResourceAsStream(fileName));
    }
    catch (IOException e)
    {
      e.printStackTrace();
    ...
    BufferedImage result = new BufferedImage(sizeX,
                                             sizeY,
  ...
  public JOCLSimpleImage()
  {
    ...
    inputImage = createBufferedImage(this.getClass(), fileName);
    imageSizeX = inputImage.getWidth();
    imageSizeY = inputImage.getHeight();

    outputImage = new BufferedImage(imageSizeX,
                                    imageSizeY,
                                    BufferedImage.TYPE_INT_RGB);
  ...
  /**
   * Starts the thread which will advance the animation state and call call the
   * animation method.
   *
  ...
  private void initImageMem()
  {
    ...
    inputImageMem = clCreateImage2D(context,
    ...
    outputImageMem = clCreateImage2D(context,
                                     CL_MEM_WRITE_ONLY,
                                     new cl_image_format[]
                                     { imageFormat },
                                     imageSizeX,
  ...
  void rotateImage(float angle)
  {
    ...
    clSetKernelArg(kernel, 2, Sizeof.cl_float, Pointer.to(new float[]
    { angle }));
