...
public class ClearCLBackendJOCL implements ClearCLBackendInterface
{
	...
	@Override
	public long getDeviceInfo(final ClearCLPeerPointer pDevicePointer,
														final DeviceInfo pDeviceSpec)
	{
		switch (pDeviceSpec)
		{
																									...
																									final long pBufferSize)
	{
		return BackendUtils.checkExceptions(() -> {
			...
			final boolean lOpenCL1p1o0 = (lDeviceVersion.contains("1.0")
																		|| lDeviceVersion.contains("1.1"));

			final long lMemFlags = BackendUtils.getMemTypeFlags(pMemAllocMode,
																													pHostAccessType,
																								...
																								final long... pDimensions)
	{
				...
				|| lDeviceVersion.contains("1.1"))
		{
			...
			final long image_height =
															(int) (pDimensions.length < 2	? 1
																														: pDimensions[1]);
			final long image_depth = (int) (pDimensions.length < 3	? 1
																															: pDimensions[2]);
				...
				|| lDeviceVersion.contains("2.0"))
		{
			...
			final cl_mem lImageMem = CL.clCreateImage((cl_context) pContextPointer.getPointer(),
																								lMemFlags,
																								lImageFormat,
																								lImageDescription,
																								null,
																									...
																									final String... pSourceCode)
	{
		return BackendUtils.checkExceptions(() -> {
			...
			final cl_program program = clCreateProgramWithSource(	(cl_context) pContextPointer.getPointer(),
																														pSourceCode.length,
																														pSourceCode,
																														null,
																														null);
	...
	{
		return BackendUtils.checkExceptions(() -> {
			final int lError = clBuildProgram((cl_program) pProgramPointer.getPointer(),
																				0,
																				null,
																				(pOptions == null
																									...
																									final String pKernelName)
	{
		return BackendUtils.checkExceptions(() -> {
			...
			final cl_kernel kernel = clCreateKernel((cl_program) pProgramPointer.getPointer(),
																							pKernelName,
																							lErrorCode);
			BackendUtils.checkOpenCLErrorCode(lErrorCode[0]);
